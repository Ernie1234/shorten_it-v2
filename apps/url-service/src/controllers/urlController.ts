...
export const createUrlSchema = z.object({
  originalUrl: z.string().url('Invalid URL format.').min(1, 'Original URL is required.'),
});
...
export const shortenUrl = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!userId) {
      return next(new ApiError(401, 'Authentication required to view your URLs.'));
    }

    logger.info(`creating URLs for userId: ${userId}`);
...
export const getUserUrls = async (req: Request, res: Response, next: NextFunction) => {
  try {
    // Get userId from custom header sent by API Gateway
    const userId = req.headers['x-user-id'] as string;
    if (!userId) {
      return next(new ApiError(401, 'Authentication required to view your URLs.'));
    }

    logger.info(`Fetching URLs for userId: ${userId}`);
...
export const redirectToOriginalUrl = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    logger.info(`Redirecting to original URL for shortCode: ${shortCode}`);
